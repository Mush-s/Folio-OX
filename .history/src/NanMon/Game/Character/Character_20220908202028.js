...
const Character = (props) => {
  const [random, setRandom] = useState(props.char, true);
  console.log(random);
  let a;
  if (random === props.char) {
    a = "START";
  } else {
    a = "NEXT";
  }
  ...
  const reloadHandler = (e) => {
    e.preventDefault();
    setRandom(props.char[Math.floor(Math.random() * props.char.length)], false);
  ...
  return (
    <>
      ...
      <div className="inputs">
        ...
        <button onClick={reloadHandler}>{a}</button>
